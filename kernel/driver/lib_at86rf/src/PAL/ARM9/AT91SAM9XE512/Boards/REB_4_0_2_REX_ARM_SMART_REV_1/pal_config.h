 ...
 *
 * $Id: pal_config.h,v 1.3 2012-02-13 01:31:11 essusige Exp $
 *
 */
/**
 * @author
...
#define F_CPU                           (100000000UL)//(48000000UL)(100 MHz)
#define F_CPUM                          (100UL)
...
/* PLL multiplier to generate PLL clock of 200Mhz. */
#define PLL_MULTIPLIER                  (96)

/* PLL divider to generate PLL clock of 200Mhz. */
#define PLL_DIVIDER                     (9)

/* SPI baud rate divider to generate 4MHz SPI clock, when F_CPU is 48MHz. */
#define SPI_BR_DIVIDER                  (25)
 ...
 */
//---------------------------------------------------------------------------------
#define PMC_PRESCALER                   (AT91C_PMC_PRES_CLK)
...
#define TRX_INTERRUPT_PIN               (AT91C_PIO_PC6)//(AT91C_PC12_IRQ0)
...
#define AIC_CONFIGURE(peripheral_id,mode, handler) do {                    \
    ...
    AT91C_BASE_AIC->AIC_SVR[peripheral_id] = (unsigned int) handler;            \
...
#define ENABLE_TRX_IRQ(trx_irq_num)     do {            \
    ...
    {                                                   \
        AT91C_BASE_AIC->AIC_IECR = _BV(AT91C_ID_IRQ0/*AT91C_ID_PIOC*/);  \
    }                                                   \
} while (0)
...
#define DISABLE_TRX_IRQ(trx_irq_num)    do {            \
    if (TRX_MAIN_IRQ_HDLR_IDX == trx_irq_num)           \
    {                                                   \
        AT91C_BASE_AIC->AIC_IDCR = _BV(AT91C_ID_IRQ0/*AT91C_ID_PIOC*/);  \
    }                                                   \
} while (0)
...
#define CLEAR_TRX_IRQ(trx_irq_num)      do {            \
    if (TRX_MAIN_IRQ_HDLR_IDX == trx_irq_num)           \
    {                                                   \
        AT91C_BASE_AIC->AIC_ICCR = _BV(AT91C_ID_IRQ0/*AT91C_ID_PIOC*/);  \
    }                                                   \
} while (0)


/*
 * Macros dealing with the global interrupt (IRQ bit) of AT91SAM9XE512.
 */
 ...
 * current status register and then disables the interrupt.
 */
#define ENTER_CRITICAL_REGION()         ubik_entercrit()

/*
 * This macro is used to mark the end of a critical region. It restores the
 * current status register.
 */
#define LEAVE_CRITICAL_REGION()         ubik_exitcrit()
 ...
 */
#define ENTER_TRX_REGION()      { uint8_t irq_mask = AT91C_BASE_AIC->AIC_ICCR; AT91C_BASE_AIC->AIC_IDCR = _BV(/*AT91C_ID_PIOC*/AT91C_ID_IRQ0)
 ...
 */
#define AT91C_BASE_SPI_USED             (AT91C_BASE_SPI0/*AT91C_BASE_SPI1*/)



/* RESET pin is pin 9 of PIOA. */
#define RST                             (AT91C_PIO_PB5/*AT91C_PIO_PA9*/)

/* Sleep Transceiver pin is pin 8 of PIOA. */
#define SLP_TR                          (AT91C_PIO_PB19/*AT91C_PIO_PA8*/)

/*
 * Slave select pin is PA21
 */
#define SEL                             (AT91C_PA3_SPI0_NPCS0/*AT91C_PA21_SPI1_NPCS0*/)

/*
 * SPI Bus Master Output/Slave Input pin is PA23
 */
#define MOSI                            (AT91C_PA1_SPI0_MOSI/*AT91C_PA23_SPI1_MOSI*/)

/*
 * SPI Bus Master Input/Slave Output pin is PA24
 */
#define MISO                            (AT91C_PA0_SPI0_MISO/*AT91C_PA24_SPI1_MISO*/)

/*
 * SPI serial clock pin is PA22
 */
#define SCK                             (AT91C_PA2_SPI0_SPCK/*AT91C_PA22_SPI1_SPCK*/)
 ...
 */
#define AT91C_BASE_SPI_USED             (AT91C_BASE_SPI0/*AT91C_BASE_SPI0*/)



/* RESET pin is pin 9 of PIOA. */
#define RST                             (AT91C_PIO_PB5/*AT91C_PIO_PA6*/)

/* Sleep Transceiver pin is pin 8 of PIOA. */
#define SLP_TR                          (AT91C_PIO_PB19/*AT91C_PIO_PA7*/)

/*
 * Slave select pin is PA15
 */
#define SEL                             (AT91C_PA3_SPI0_NPCS0/*AT91C_PA12_SPI0_NPCS0*/)

/*
 * SPI Bus Master Output/Slave Input pin is PA17
 */
#define MOSI                            (AT91C_PA1_SPI0_MOSI/*AT91C_PA17_SPI0_MOSI*/)

/*
 * SPI Bus Master Input/Slave Output pin is PA16
 */
#define MISO                            (AT91C_PA0_SPI0_MISO/*AT91C_PA16_SPI0_MISO*/)

/*
 * SPI serial clock pin is PA18
 */
#define SCK                             (AT91C_PA2_SPI0_SPCK/*AT91C_PA18_SPI0_SPCK*/)
 ...
 * Set TRX GPIO pins.
 */
#define RST_HIGH()                      {AT91C_BASE_PIOB->PIO_SODR = RST;/*AT91C_BASE_PIOA->PIO_SODR = RST;*/}
#define RST_LOW()                       {AT91C_BASE_PIOB->PIO_CODR = RST;/*AT91C_BASE_PIOA->PIO_CODR = RST;*/}
#define SLP_TR_HIGH()                   {AT91C_BASE_PIOB->PIO_SODR = SLP_TR;/*AT91C_BASE_PIOA->PIO_SODR = SLP_TR;*/}
#define SLP_TR_LOW()                    {AT91C_BASE_PIOB->PIO_CODR = SLP_TR;/*AT91C_BASE_PIOA->PIO_CODR = SLP_TR;*/}
...
#define PAL_WAIT_1_US()                 {nop(); nop(); nop(); nop(); \
                                         ...
                                         nop(); nop(); nop(); nop(); \
                                         nop(); nop(); nop(); nop();}
#elif (F_CPU == 100000000UL)//이쪽은 타이밍이 안맞으므로 문제가 생길 가능성이 있음.
	/* 100 MHz */
	/*
	 * Wait for 65 ns.
	 * time t7: SLP_TR time (see data sheet or SWPM).
 ...
 */
#define LED_PINS                (AT91C_PIO_PA23 | AT91C_PIO_PA24 )
//LED 없음.
 ...
 */
#define ALERT_INIT()        {AT91C_BASE_PIOA->PIO_OER = LED_PINS;}
//LED도 없고 ALERT도 모르겠고
/*
 * Alert indication
 */
