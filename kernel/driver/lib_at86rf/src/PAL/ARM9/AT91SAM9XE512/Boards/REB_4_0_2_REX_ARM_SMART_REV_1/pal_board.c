 ...
 *
 * $Id: pal_board.c,v 1.3 2012-02-13 01:31:11 essusige Exp $
 *
 *  @author
...
void trx_interface_init(void)
{
     ...
     * Peripheral B.
     */
	AT91C_BASE_PMC->PMC_PCER = _BV(AT91C_ID_PIOA | AT91C_ID_PIOB | AT91C_ID_PIOC);
#if 	   (1 == EXCLUDE_MODIFICATION_FOR_UBINOS)
	AT91C_BASE_PIOA->PIO_IDR = (MISO | MOSI | SCK);
	...
	AT91C_BASE_PMC->PMC_PCER = _BV(AT91C_ID_PIOC);
	AT91C_BASE_PIOC->PIO_IDR = TRX_INTERRUPT_PIN;
	AT91C_BASE_PIOC->PIO_PPUDR = TRX_INTERRUPT_PIN;
	AT91C_BASE_PIOC->PIO_IFDR = TRX_INTERRUPT_PIN;
	AT91C_BASE_PIOC->PIO_ODR = TRX_INTERRUPT_PIN;//Output disable, PIO Interrupt를 쓰므로 Input으로 설정
	AT91C_BASE_PIOC->PIO_PER = TRX_INTERRUPT_PIN;
...
//	AT91C_BASE_PIOC->PIO_PPUDR = TRX_INTERRUPT_PIN;
//    // Configure pin
//	AT91C_BASE_PIOC->PIO_ASR = TRX_INTERRUPT_PIN;
//	AT91C_BASE_PIOC->PIO_PDR = TRX_INTERRUPT_PIN;
//--------------------------IRQ사용시-------------------------------
    ...
    AT91C_BASE_PMC->PMC_PCER = _BV(AT91C_ID_SPI0);
#endif	/* (1 == EXCLUDE_MODIFICATION_FOR_UBINOS) */
    AT91C_BASE_SPI_USED->SPI_CR = AT91C_SPI_SWRST;
    AT91C_BASE_SPI_USED->SPI_CR = AT91C_SPI_SWRST;
    /* The controller is configured to be master. */
    AT91C_BASE_SPI_USED->SPI_MR = (AT91C_SPI_MSTR | AT91C_SPI_MODFDIS | AT91C_SPI_PCS);
    AT91C_BASE_SPI_USED->SPI_PTCR = (AT91C_PDC_RXTDIS | AT91C_PDC_TXTDIS);
    AT91C_BASE_SPI_USED->SPI_CR = AT91C_SPI_SPIEN;
    AT91C_BASE_PMC->PMC_PCDR = _BV(AT91C_ID_SPI0);
     ...
     */
    int SMART_Board_MCK = ((18432000 * 97 / 9) / 2);
    int SMART_SPCK = 2000000;
    AT91C_BASE_SPI_USED->SPI_CSR[0] = (AT91C_SPI_NCPHA | SPID_CSR_DLYBCT(SMART_Board_MCK, 250) \
        	     | SPID_CSR_DLYBS(SMART_Board_MCK, 250) | SPID_CSR_SCBR(SMART_Board_MCK, SMART_SPCK)) \
        	     | AT91C_SPI_BITS_8;
    ...
    int reg;
    reg = AT91C_BASE_SPI0->SPI_MR;
   	reg &= (~(AT91C_SPI_PCS));
   	AT91C_BASE_SPI0->SPI_MR = reg;
}
...
void gpio_init(void)
{
    ...
    AT91C_BASE_PIOB->PIO_OER = RST;
    AT91C_BASE_PIOB->PIO_OER = SLP_TR;
    AT91C_BASE_PIOA->PIO_OER = SEL;
    AT91C_BASE_PIOB->PIO_PER = RST;
    AT91C_BASE_PIOB->PIO_PER = SLP_TR;
    AT91C_BASE_PIOA->PIO_OER = SEL;
}
...
//    AT91C_BASE_PIOA->PIO_OER = LED_PINS;
//    AT91C_BASE_PIOA->PIO_SODR = LED_PINS;
}
...
//    switch (led_no)
//    {
//        case LED_0: led_pin = AT91C_PIO_PA23; break;
//        case LED_1: led_pin = AT91C_PIO_PA24; break;
//        default: return;
//    }
//
//    /* LEDs are low active. */
...
//    else    /* LED_TOGGLE */
//    {
//        if (AT91C_BASE_PIOA->PIO_ODSR & led_pin)
//        {
//            AT91C_BASE_PIOA->PIO_CODR = led_pin;
//        }
//        else
...
#endif /* REB_4_0_2_REX_ARM_SMART_REV_1 */
