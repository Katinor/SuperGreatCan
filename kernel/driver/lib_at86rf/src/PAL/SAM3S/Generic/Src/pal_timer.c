 ...
 *
 * $Id: pal_timer.c,v 1.1 2012-02-13 01:31:11 essusige Exp $
 *
                         ...
                         void *param_cb)
{
    ...
    timer_count = (uint32_t) USEC_TO_TIMERTICK(timer_count);
...
retval_t pal_stop_high_priority_timer(uint8_t timer_id)
{
    ...
    if (timer_id == high_priority_timer_id)
    {
         ...
         */
        PERIPHERAL_CLOCK_DISABLE(AT91C_ID_TC1);
...
#else 	/* (1 == EXCLUDE_MODIFICATION_FOR_UBINOS) */
    *current_time = (uint32_t)(TIMERTICK_TO_USEC(gettime()));
#endif	/* (1 == EXCLUDE_MODIFICATION_FOR_UBINOS) */
}
...
//#else 	/* (1 == EXCLUDE_MODIFICATION_FOR_UBINOS) */
//    delay = (uint32_t) USEC_TO_TIMERTICK(delay);
//#endif	/* (1 == EXCLUDE_MODIFICATION_FOR_UBINOS) */
//    uint16_t target_time = TC0->TC_CHANNEL->TC_CV + delay;
//
//    if ( target_time > (TC0->TC_CHANNEL->TC_CV))
//    {
//        /* Target time is within current span of 16 bit count (0xFFFF). */
//        while (target_time > (TC0->TC_CHANNEL->TC_CV))
//        {
...
static inline uint32_t gettime(void)
{
    ...
    do
    {
        ...
        current_time = current_time | TC0->TC_CHANNEL->TC_CV;
        /*
...
static void prog_reg_rc(void)
{
    ...
    if (NO_TIMER != running_timer_queue_head)
    {
        ...
        if (timeout_high == sys_time)
        {
            ...
            /* The compare register is programmed */
            TC0->TC_CHANNEL->TC_RC = timeout_low;
        ...
        else
        {
             ...
             * disabled.
             */
        	TC0->TC_CHANNEL->TC_IDR = TC_IDR_CPCS;
...
void timer_ch0_irq_handler(void)
{
     ...
     * the interrupt.
     */
    irq_status = TC0->TC_CHANNEL->TC_SR;

#if (TOTAL_NUMBER_OF_TIMERS > 0)
    if (TC_SR_CPCS & irq_status)
    {
        /*
