 ...
 *
 * $Id: pal.c,v 1.1 2012-02-13 01:31:11 essusige Exp $
 *
 ...
 */
#define BOARD_OSCOUNT           (CKGR_MOR_MOSCXTST & (0x40 << 8))
 ...
 */
#define BOARD_PLLCOUNT          (CKGR_PLLAR_PLLACOUNT & (16 << 8))

/* The PLL output is = ((MUL + 1) / DIV). */
#define BOARD_MUL               (CKGR_PLLAR_MULA & (PLL_MULTIPLIER << 16))
#define BOARD_DIV               (CKGR_PLLAR_DIVA & PLL_DIVIDER)

/*
 * The value to be written in the CSS field of the PMC_MCKR register to
 * select the PLL clock as the input to the PMC.
 */
#define PLL_CLOCK_FOR_PMC       (PMC_MCKR_CSS_PLLA_CLK)
