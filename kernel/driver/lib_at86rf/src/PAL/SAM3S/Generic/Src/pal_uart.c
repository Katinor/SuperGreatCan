 ...
 *
 * $Id: pal_uart.c,v 1.1 2012-02-13 01:31:11 essusige Exp $
 *
...
#if ((defined PAL_UART0) || (defined PAL_UART1))
#include <stdint.h>
#include "../../../Inc/pal.h"
#include "../../../../Include/_pal_config.h"
#include "../Inc/pal_uart.h"
...
void sio_uart_0_init(uint32_t baud_rate)
{
    ...
    /* ID of the PAL_UART0 peripheral. */
    uart_id = ID_USART0;

    /* Pointer to the PAL_UART0 base. */
    //&uart_ptr = USART0;

    /* Interrupt handler for PAL_UART0. */
     ...
     */
 //   PIOA->PIO_ASR = (PIN_UART_0_RXD | PIN_UART_0_TXD);
    abcdsr = PIOA->PIO_ABCDSR[0];
    PIOA->PIO_ABCDSR[0] &= ((~(PIN_UART_0_RXD | PIN_UART_0_TXD))& abcdsr);
    abcdsr = PIOA->PIO_ABCDSR[1];
    PIOA->PIO_ABCDSR[1] &= ((~(PIN_UART_0_RXD | PIN_UART_0_TXD))& abcdsr);

    PIOA->PIO_PDR = (PIN_UART_0_RXD | PIN_UART_0_TXD);


    /* The clock for selected UART channel is enabled. */
    PMC->PMC_PCER0 = _BV(uart_id);
     ...
     * US_BRGR register.
     */
    USART0->US_BRGR = UART_BAUD_DIVISOR;

    /* The AIC is set up for the UART interrupts. */
//    NVIC_CONFIGURE(uart_id, AT91C_AIC_SRCTYPE_INT_POSITIVE_EDGE, irq_handler);

   	do {
    	    /* The interrupts for specified peripheral are first disabled. */
    		intr_disable(uart_id);
    	    /*
    		...
    		intr_connectisr(uart_id, (void *)irq_handler, 0x40, 0);
    	    /* Pending interrupts if any for the peripheral are cleared. */
    		intr_ClearPending(uart_id);
    	} while (0);
...
void sio_uart_1_init(uint32_t baud_rate)
{
    ...
    /* ID of the PAL_UART1 peripheral. */
    uart_id = AT91C_ID_US1;

    /* Pointer to the PAL_UART1 base. */
    uart_ptr = AT91C_BASE_US1;

    /* Iinterrupt handler for PAL_UART1. */
...
#ifdef PAL_UART0
/**
 * @brief ISR for PAL_UART0 transmission and reception interrupts
 *
 * This service routine is executed when a byte is successfully transmitted or
 * received on PAL_UART0.
 */
void uart0_irq_handler(void)
{
     ...
     */
    uart_irq_cause = USART0->US_CSR;

    if (US_IER_RXRDY & uart_irq_cause)
    {
        uint8_t tail = uart_0_buffer.rx_buf_tail;

        /* Count of bytes received through PAL_UART0 channel is incremented. */
        uart_0_buffer.rx_count++;
    ...
    else if (US_IER_TXRDY & uart_irq_cause)
    {
        /*
         * PAL_UART0 transmit interrupt is disabled.
...
#ifdef PAL_UART1
/**
 * @brief ISR for PAL_UART1 transmission and reception interrupts
 *
 * This service routine is executed when a byte is successfully transmitted or
 * received on PAL_UART1.
 */
void uart1_irq_handler(void)
{
    ...
    if (AT91C_US_RXRDY & uart_irq_cause)
    {
        ...
        /* Count of bytes received through PAL_UART1 channel is incremented. */
        uart_1_buffer.rx_count++;
    ...
    {
        /*
         * PAL_UART1 transmit interrupt is disabled.
...
#endif  /* ((defined PAL_UART0) || (defined PAL_UART1)) */
