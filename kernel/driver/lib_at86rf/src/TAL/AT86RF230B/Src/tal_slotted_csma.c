 ...
 *
 * $Id: tal_slotted_csma.c,v 1.5 2010-08-05 05:41:48 slb Exp $
 *
...
static void send_frame_at_next_backoff_boundary(void)
{
    ...
    pal_trx_irq_init(TRX_MAIN_IRQ_HDLR_IDX, (FUNC_PTR)ack_reception_handler_cb);
    pal_trx_irq_enable(TRX_MAIN_IRQ_HDLR_IDX);
    ...
    ack_is_requested = *(tal_frame_to_tx + 1) & FCF_ACK_REQ;
            ...
            case WAITING_FOR_ACK:
                {
                     ...
                     * load the frame from the transceiver.
                     */
                    ...
                    if (ack_frame[3] == tal_frame_to_tx[SEQ_NUMBER_POS])
                    {
                        ...
                        if ((ack_frame[1] & FCF_FRAME_PENDING_MASK) ==
                            FCF_FRAME_PENDING_MASK)
                        {
